...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'config'], function (exports) {
    "use strict";
    ...
    var $ = layui.$
        ...
        , TPL_HEADER = function (options) {
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                , '<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' + rowCols + ' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(item2.css){ }} {{item2.css}} {{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
        ...
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    if (res && config.ajaxSuccessBefore(res) == false) {
                        return;
                    }
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        that.renderData(res, curr, res.data[response.countName]), sort();
                        options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
                    }
                    that.setColsWidth();
                    typeof options.done === 'function' && options.done(res, curr, res.data[response.countName]);
                }
                , error: function (e, m) {
                    if (e.status == 500) {
                        ...
                        this.success({code: e.status, msg: e.statusText});
                    }
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            , options = that.config
            , data = res[options.response.dataName] || res.data[options.response.dataName] || []
            , trs = []
            , trs_fixed = []
            , trs_fixed_r = []
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    that.eachCols(function (i3, item3) {
                        ...
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { //追加各种属性
                            ...
                            if (item3.css) classNames.push(item3.css); //加入特定css
